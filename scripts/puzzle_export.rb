...
DATA_DIR = FLAGS[:dir] ? "#{Dir.getwd}/#{FLAGS[:dir]}" : "#{__dir__}/../src/scripts/data"

puzzles_file = File.open("#{DATA_DIR}/puzzles.js", "w")
i = 0
Dir.each_child(DATA_DIR) do |file|
  if file =~ /puzzle_\d+\.js$/
    i += 1
    puzzles_file.write("import p#{i} from './#{file}'\n")
  end
end

puzzles_file.write("\nexport default [\n")

exports = (1..i).map { |n| "\tp#{n},\n" }
puzzles_file.write(exports.join)
